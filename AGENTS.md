...
- ❌ No `kubectl`, `clusterctl`, `flux bootstrap`, cloud CLIs, or Proxmox API calls.
- ❌ No plaintext secrets. Use placeholders or SOPS-encrypted examples only.
- ❌ No architectural changes without explicit instruction: follow `README.md` and `/docs`.
- ❌ No long-running background tasks; every change must be self-contained in the PR.
...
- **Defaults:** read `clusters/<provider>/values.example.yaml`; merge flag overrides → values map.
...
- ✅ `go build` succeeds; YAML parses; docs (if changed) build with `mkdocs --strict`.
- ✅ Files live in correct directories and follow naming/indentation rules.
- ✅ No plaintext secrets; examples use placeholders or SOPS-encrypted files.
