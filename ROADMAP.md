# ✅ InfraFlux — GitHub Checklist

> Track your build from bare metal to GitOps nirvana. Check things off as you go.

---

## 1) Preflight & Foundations

- [x] **1.1 Validate infra assumptions**

  - [x] **Docs:** [Preflight checklist](docs/preflight.md), [Secrets inventory & mapping](docs/secrets.md)
  - [x] **Status:** Preflight doc authored; secrets mapping drafted; links wired.
...
> - MinIO without persistent storage or wrong endpoint scheme (http/https) → backend init & Velero misbehave. ([velero.io][5])

---

## 2) Image Build (Talos Image Factory)

- [x] **2.0 Docs:** [Image Factory — schematic & build](docs/image-factory.md)
- [x] **2.0 Status:** Image uploaded and tested.

- [x] **2.1 Author Talos Image Factory schematic (universal image)**

  - [x] **2.1.1** Include system extensions: `siderolabs/qemu-guest-agent`, `siderolabs/iscsi-tools`, `siderolabs/util-linux-tools`, plus Intel microcode. ([GitHub][7], [factory.talos.dev][8])
  - [x] **2.1.2** Choose target = nocloud; pin Talos version (latest stable).
  - [x] **2.1.3** Export the image URL/ID for Terraform.

- [x] **2.2 Sanity test the image in Proxmox (manual one-off VM)**
  - [x] **2.2.1** Boot, verify guest agent, verify `iscsiadm` present, verify util-linux tools.
  - [x] **2.2.2** Confirm console shows Talos ready.

> ⚠️ **Pitfalls (Step 2)**
>
> - Missing `iscsi-tools`/`util-linux-tools` → Longhorn won’t mount volumes. ([Longhorn][9])
> - Occasional first-boot quirks on PVE—retry or reprovision; watch Talos/Proxmox first-boot notes. ([GitHub][10])
...
- [x] **3.1 Lay out repo**
  ...
  - [x] **3.1.1** `terraform/modules/*` for: `proxmox-vm`, `talos-cluster`, `cilium`, `argocd`, `external-secrets`, `longhorn`, `cert-manager`, `external-dns`, `observability`.
  - [x] **3.1.2** `terraform/envs/prod` with `main.tf`, `providers.tf` (bpg/proxmox, helm, kubernetes), `backend.tf` (S3→MinIO), `versions.tf`, `variables.tf`, `prod.auto.tfvars.example`.
  - [x] **3.1.3** `.github/workflows`: `plan.yml` (fmt/validate/plan), `apply.yml` (manual approval).

- [x] **3.2 Configure Terraform backend to MinIO**

  - [x] **3.2.1** `backend "s3"`: endpoint, bucket, access/secret (from 1Password at runtime).
  - [x] **3.2.2** Enable locking if supported.

- [x] **3.3 Provision a self-hosted GitHub runner (on LAN)**
  - [x] **3.3.1** Ensure network reachability to Proxmox and MinIO; least-priv secrets.

> ⚠️ **Pitfalls (Step 3)**
>
> - Storing state in Git by accident—ensure S3 backend works before `apply`.
> - Wrong MinIO endpoint scheme (need `skip_credentials_validation`/`skip_region_validation`) → init fails.
...
> ⚠️ **Pitfalls (Step 5)**
>
> - Installing CNI before Talos bootstrap completes → API flaps.
...
- [x] **7.2 App-of-Apps + ApplicationSets**
  - [x] **7.2.1** Parent “root” Application created (temporary path `old/gitops/apps` for examples). ([Argo CD][18])
  - [x] **7.2.2** ApplicationSets: Git directory generator scaffold added (`old/gitops/applicationsets/apps-dir.yaml`). Matrix to follow when multi-env arrives. ([Argo CD][19])

> ⚠️ **Pitfalls (Step 7)**
>
> - App-of-Apps is admin-powerful; restrict who can edit the parent app repo. ([Argo CD][18])
> - Ordering: ensure CRDs (ESO, cert-manager) sync **before** dependents (use sync waves/health checks).

---

## 8) Secrets: ESO + 1Password SDK

- [x] **8.1 Install ESO + 1Password SDK provider**

  - [x] **8.1.1** Create `SecretStore` referencing 1Password SDK service token & vault (scaffolded; token populated out-of-band).
  - [x] **8.1.2** Create `ExternalSecret` objects for: Cloudflare token (cert-manager, external-dns), ArgoCD OIDC client secret scaffolds present. ([external-secrets.io][6])

- [x] **8.2 Wire workloads to K8s Secrets generated by ESO**
  - [x] **8.2.1** Argo CD values reference ESO-managed secret via `$argo:oidc.clientSecret` and ExternalSecret writes `argocd-secret` key `oidc.clientSecret`.
  - [x] **8.2.2** cert-manager ClusterIssuer and external-dns consume ESO-provisioned Cloudflare token; sync-waves ensure order.

> ⚠️ **Pitfalls (Step 8)**
>
> - Mixing SOPS + ESO → confusion; stick to ESO-only.
> - Wrong SDK token scope or vault name → SecretStore errors. ([external-secrets.io][6])
> - Ordering matters: we annotate ESO secrets at wave -1, issuers at 0, consumers at 1 to avoid race conditions.

---

## 9) DNS, TLS & Certificates

- [x] **9.1 cert-manager with Cloudflare DNS-01**

  - [x] **9.1.1** Install cert-manager CRDs & controller (Argo App with installCRDs=true + CreateNamespace).
  - [x] **9.1.2** Create ClusterIssuer using **Cloudflare API Token** (ESO-provisioned Secret; sync-wave ordering applied). ([cert-manager][20])

- [x] **9.2 external-dns (Cloudflare)**
  - [x] **9.2.1** Configure provider = Cloudflare, domainFilters, and token from ESO; namespace auto-created. ([Kubernetes SIGs][21], [GitHub][22])
...
- [ ] **15.1 Apply parent app (App-of-Apps)**
  ...
  - [ ] **15.1.2** Verify health checks and sync waves to avoid “apply grenade.” ([Argo CD][18])
